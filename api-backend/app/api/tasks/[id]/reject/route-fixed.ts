...
export async function POST(
  ...
  try {
    ...
    // Check if user is assigned to this task (either as single assignee or in task_assignees table)
    const { rows: assigneeRows } = await db.query(
      'SELECT user_id FROM task_assignees WHERE task_id = $1 AND user_id = $2',
      [taskId, user.id]
    );

    const isSingleAssignee = task.assigned_to_id === user.id;
    const isInAssigneesTable = assigneeRows.length > 0;

    if (!isSingleAssignee && !isInAssigneesTable) {
