...
export default function DashboardPage() {
  ...
  const [taskToRefuse, setTaskToRefuse] = useState<Task | null>(null)
  const { user } = useAuth()
  ...
  useEffect(() => {
    const rejectTask = searchParams.get('reject_task')
    const taskId = searchParams.get('taskId')

    if (rejectTask === 'true' && taskId && user) {
      // Fetch the task and show the modal
      const fetchTask = async () => {
        try {
          const response = await dashboardApi.getTaskById(taskId)
          setTaskToRefuse(response.data)
